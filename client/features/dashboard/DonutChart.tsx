                ...
                <div className="flex-1 space-y-2 text-sm min-w-0">
                    {data.slice(0, 5).map((item, index) => (
                        <div key={`${item.name}-${index}`} className="flex items-center gap-2">
                            ...
                            <span className="font-medium text-on-surface flex-1 truncate">{item.name}</span>
                            <span className="font-semibold text-on-surface">{item.value.toLocaleString('en-IN')}</span>
                            <span className="text-muted w-10 text-right">{((item.value / total) * 100).toFixed(0)}%</span>
                        </div>
                    ))}
